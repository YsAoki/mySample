...
const SearchForm: FC = ({ ...other }) => {
  ...
  const [startPrice, setStartPrice] = useState(defaultState.matchStart);
  ...
  const [endPrice, setEndPrice] = useState(defaultState.matchEnd);
  ...
  const onChangeIsPopular = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value as "popular" | "notPopular";
    const newIsPopular = { popular: false, notPopular: false };
    newIsPopular[value] = true;
  ...
  console.log(getCheckedItemsArray());
    ...
    <SForm {...other} className={searchModalSwitch ? "active" : undefined}>
      ...
      <SInner>
        ...
        <RadioButton
          ...
          id="popular"
          value="popular"
          onChange={onChangeIsPopular}
          checked={isPopular.notPopular}
        >
          人気あり
        </RadioButton>
        <RadioButton
